...
#define request2size(req) \
	(((req) + 2 * SIZE_SZ + MALLOC_ALIGN_MASK < MINSIZE) ? MINSIZE : ((req) + 2 * SIZE_SZ + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK)
#else
#define request2size(req) \
...
#define inuse_bit_at_offset(p, offset) (((MallocChunk *)(((char *)p) + offset))->m_tSize & PREV_INUSE)
#define set_inuse_bit_at_offset(p, s) (((MallocChunk *)(((char *)(p)) + (s)))->m_tSize |= PREV_INUSE)
#define clear_inuse_bit_at_offset(p, s) (((MallocChunk *)(((char *)(p)) + (s)))->m_tSize &= ~(PREV_INUSE))
#define set_size_at_offset(p, offset, size) (((MallocChunk *)(((char *)p) + (offset)))->m_tSize = REAL_SIZE(size) | (((MallocChunk *)(((char *)p) + (offset)))->m_tSize & SIZE_BITS))
#define set_presize_at_offset(p, offset, size) (((MallocChunk *)(((char *)p) + (offset)))->m_tPreSize = REAL_SIZE(size))
...
#define largebin_index(sz) \
	(((((unsigned long)(sz)) >> 6) <= 32) ? 56 + (((unsigned long)(sz)) >> 6) : ((((unsigned long)(sz)) >> 9) <= 20) ? 91 + (((unsigned long)(sz)) >> 9) : ((((unsigned long)(sz)) >> 12) <= 10) ? 110 + (((unsigned long)(sz)) >> 12) : ((((unsigned long)(sz)) >> 15) <= 4) ? 119 + (((unsigned long)(sz)) >> 15) : ((((unsigned long)(sz)) >> 18) <= 2) ? 124 + (((unsigned long)(sz)) >> 18) : 126)
...
DTCBinMalloc::DTCBinMalloc()
{
	...
	statChunkTotal = statmgr.get_item_u32(DTC_CHUNK_TOTAL);
	statDataSize = statmgr.get_item(DTC_DATA_SIZE);
	statMemoryTop = statmgr.get_item(DTC_MEMORY_TOP);
	statTmpDataSizeRecently = 0;
	statTmpDataAllocCountRecently = 0;
	statAverageDataSizeRecently = statmgr.get_item(DATA_SIZE_AVG_RECENT);
...
DTCBinMalloc *DTCBinMalloc::Instance()
{
	return Singleton<DTCBinMalloc>::Instance();
}

void DTCBinMalloc::Destroy()
{
	Singleton<DTCBinMalloc>::Destroy();
}
/*初始化header中的signature域*/
void DTCBinMalloc::init_sign()
{
...
int DTCBinMalloc::Init(void *pAddr, INTER_SIZE_T tSize)
{
	int i;

	if (tSize < sizeof(MemHead) + sizeof(CBin) * (NBINS + NFASTBINS + 1) + DTC_RESERVE_SIZE + MINSIZE)
	{
	...
	m_pstHead->m_hReserveZone = sizeof(MemHead) + sizeof(CBin) * (NBINS + NFASTBINS + 1);
	m_pstHead->m_hReserveZone = (m_pstHead->m_hReserveZone + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK;
	m_pstHead->m_hBottom = (m_pstHead->m_hReserveZone + DTC_RESERVE_SIZE + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK;
...
/*校验cache的版本是否正确*/
int DTCBinMalloc::detect_version()
{
	if (m_pstHead->m_auiSign[0] != DTC_SIGN_0 || m_pstHead->m_auiSign[1] != DTC_SIGN_1)
		return 1;
...
int DTCBinMalloc::share_memory_integrity()
{
	return (int)m_pstHead->m_shmIntegrity;
}

void DTCBinMalloc::set_share_memory_integrity(const int flags)
{
	if (flags)
...
int DTCBinMalloc::Attach(void *pAddr, INTER_SIZE_T tSize)
{

	if (tSize < sizeof(MemHead) + sizeof(CBin) * (NBINS + NFASTBINS + 1) + MINSIZE)
	{
		snprintf(m_szErr, sizeof(m_szErr), "invalid size[" UINT64FMT_T "]", tSize);
		return (-1);
	}
...
ALLOC_HANDLE_T DTCBinMalloc::get_reserve_zone()
{
	return m_pstHead->m_hReserveZone;
}
/*输入参数是chunk的用户handle*/
/*返回这块chunk的用户使用空间的大小*/
ALLOC_SIZE_T DTCBinMalloc::chunk_size(ALLOC_HANDLE_T hHandle)
{
	MallocChunk *pstChunk;

	if (hHandle >= m_pstHead->m_hTop || hHandle <= m_pstHead->m_hBottom)
	{
		snprintf(m_szErr, sizeof(m_szErr), "[chunk_size]-invalid handle");
		return (0);
	}
	...
	{
		snprintf(m_szErr, sizeof(m_szErr), "[chunk_size]-invalid chunk");
		return (0);
	}
...
void *DTCBinMalloc::bin_malloc(CBin &ptBin)
{
	MallocChunk *pstChunk;
	void *p;
...
void *DTCBinMalloc::fast_malloc(ALLOC_SIZE_T tSize)
{
	return bin_malloc(m_ptFastBin[smallbin_index(tSize)]);
}
/*从smallbins的一个bin下取一个空闲chunk满足tsize大小*/
void *DTCBinMalloc::small_bin_malloc(ALLOC_SIZE_T tSize)
{
	void *p;
...
int DTCBinMalloc::free_fast()
{
	...
	for (int i = 0; i < NFASTBINS; i++)
	{
		if (m_ptFastBin[i].m_hNextChunk != INVALID_HANDLE)
		{
			...
			do
			{ // free fast-chunk & put it into unsorted chunk list
				pstChunk = (MallocChunk *)handle_to_ptr(m_ptFastBin[i].m_hNextChunk);
				unlink_bin(m_ptFastBin[i], m_ptFastBin[i].m_hNextChunk);
				...
				if (!prev_inuse(pstChunk) && CAN_COMBILE(tSize, pstChunk->m_tPreSize))
				{
					...
					tSize += tPreSize;
					pstChunk = (MallocChunk *)(((char *)pstChunk) - tPreSize);
				...
				{
					pstNextChunk = (MallocChunk *)(((char *)pstChunk) + tSize);
					...
					{
						tSize += tNextSize;
						unlink_bin(m_ptBin[uiBinIdx], ptr_to_handle(pstNextChunk));
						if (empty_bin(uiBinIdx))
							clear_bin_bit_map(uiBinIdx);
						set_inuse_bit_at_offset(pstChunk, tSize);
					}
				...
				{
					link_bin(m_ptUnsortedBin[0], ptr_to_handle(pstChunk));
				}
				pstNextChunk = (MallocChunk *)(((char *)pstChunk) + REAL_SIZE(tSize));
...
void *DTCBinMalloc::top_alloc(ALLOC_SIZE_T tSize)
{
...
int DTCBinMalloc::unlink_bin(CBin &stBin, INTER_HANDLE_T hHandle)
{
	MallocChunk *pstChunk;
	MallocChunk *pstTmp;
...
int DTCBinMalloc::link_bin(CBin &stBin, INTER_HANDLE_T hHandle)
{
	MallocChunk *pstChunk;
	MallocChunk *pstTmp;
...
int DTCBinMalloc::link_sorted_bin(CBin &stBin, INTER_HANDLE_T hHandle, ALLOC_SIZE_T tSize)
{
	MallocChunk *pstChunk;
	MallocChunk *pstNextChunk;
	...
	else
	{
		...
		else
		{
			...
			if (pstChunk->m_hNextChunk != INVALID_HANDLE)
			{
				pstNextChunk = (MallocChunk *)handle_to_ptr(pstChunk->m_hNextChunk);
				pstNextChunk->m_hPreChunk = ptr_to_handle(pstChunk);
			}
			else
			{
				// list tail
...
/*分配chunk满足tsize的主体逻辑*/
ALLOC_HANDLE_T DTCBinMalloc::inter_malloc(ALLOC_SIZE_T tSize)
{
	void *p;
	...
	for (;;)
	{
		...
		if (!in_smallbin_range(tSize))
		{
			...
			else
			{
				...
				((MallocChunk *)(((char *)pstChunk) + tSize + tRemainSize))->m_tPreSize = tRemainSize;
				set_inuse_bit_at_offset(pstNextChunk, tRemainSize);
				ALLOC_SIZE_T user_size;
				inter_free(chunkhandle2memhandle(ptr_to_handle(pstNextChunk)), user_size);
			}
...
ALLOC_HANDLE_T DTCBinMalloc::Malloc(ALLOC_SIZE_T tSize)
{
	MallocChunk *pstChunk;
...
ALLOC_HANDLE_T DTCBinMalloc::Calloc(ALLOC_SIZE_T tSize)
{
...
/*当输入的chunk在使用中时候返回0*/
int DTCBinMalloc::check_inuse_chunk(MallocChunk *pstChunk)
{
	...
	{
		pstTmp = (MallocChunk *)(((char *)pstChunk) - pstChunk->m_tPreSize);
		if (ptr_to_handle(pstTmp) < m_pstHead->m_hBottom || CHUNK_SIZE(pstTmp) != pstChunk->m_tPreSize)
		{
...
/*realloc的主体逻辑*/
ALLOC_HANDLE_T DTCBinMalloc::inter_re_alloc(ALLOC_HANDLE_T hHandle, ALLOC_SIZE_T tSize, ALLOC_SIZE_T &tOldMemSize)
{
	...
	MallocChunk *pstTmp;
	MallocChunk *pstNextChunk;
	pstNextChunk = (MallocChunk *)(((char *)pstChunk) + CHUNK_SIZE(pstChunk));
	...
	else
	{
		...
		{
			pstChunk->m_tSize = REAL_SIZE(tSize) | (pstChunk->m_tSize & SIZE_BITS);
			pstNextChunk = (MallocChunk *)handle_to_ptr(m_pstHead->m_hTop + (tSize - tOldSize));
			pstNextChunk->m_tPreSize = REAL_SIZE(tSize);
		...
		else if (!AT_TOP(pstChunk, tOldSize) && !inuse_bit_at_offset(pstNextChunk, CHUNK_SIZE(pstNextChunk)) && ((INTER_SIZE_T)tOldSize + CHUNK_SIZE(pstNextChunk)) >= tSize)
		{
			...
			unlink_bin(m_ptBin[bin_index(CHUNK_SIZE(pstNextChunk))], ptr_to_handle(pstNextChunk));
		}
		/* ada: defrag */
		else if (!prev_inuse(pstChunk) && (tOldSize + pstChunk->m_tPreSize) >= tSize)
		{
			...
			unlink_bin(m_ptBin[bin_index(pstChunk->m_tPreSize)], hNewHandle);
			// copy user data
			memmove(chunk2mem(handle_to_ptr(hNewHandle)), chunk2mem(handle_to_ptr(hHandle)), chunksize2memsize(tOldSize));
		}
		else
		{
			// alloc , copy & free
			...
			pstTmp = (MallocChunk *)mem2chunk(handle_to_ptr(hNewHandle));
			hNewHandle = ptr_to_handle(pstTmp);
			tNewSize = CHUNK_SIZE(pstTmp);
			// copy user data
			memcpy(chunk2mem(pstTmp), chunk2mem(handle_to_ptr(hHandle)), chunksize2memsize(tOldSize));
	...
	if (tRemainderSize >= get_min_chunk_size())
	{
		...
		do
		{
			...
			// next chunk
			pstNextChunk = (MallocChunk *)(((char *)pstRemainChunk) + REAL_SIZE(tThisChunkSize));
			pstNextChunk->m_tPreSize = REAL_SIZE(tThisChunkSize);
			pstNextChunk->m_tSize |= PREV_INUSE;
			/* Mark remainder as inuse so free() won't complain */
			set_inuse_bit_at_offset(pstRemainChunk, tThisChunkSize);
			ALLOC_SIZE_T tTmpSize;
			inter_free(ptr_to_handle(chunk2mem(pstRemainChunk)), tTmpSize);

			tPreChunkSize = tThisChunkSize;
			tRemainderSize -= tThisChunkSize;
			pstRemainChunk = (MallocChunk *)(((char *)pstRemainChunk) + REAL_SIZE(tThisChunkSize));
	...
	{
		// next chunk
		pstNextChunk = (MallocChunk *)(((char *)pstNewChunk) + REAL_SIZE(tNewSize));
		pstNextChunk->m_tSize |= PREV_INUSE;
	}
...
ALLOC_HANDLE_T DTCBinMalloc::ReAlloc(ALLOC_HANDLE_T hHandle, ALLOC_SIZE_T tSize)
{
...
/*free接口的主体逻辑*/
int DTCBinMalloc::inter_free(ALLOC_HANDLE_T hHandle, ALLOC_SIZE_T &tMemSize)
{
	tMemSize = 0;
	if (hHandle == INVALID_HANDLE)
		return (0);
	...
	if (!prev_inuse(pstChunk) && CAN_COMBILE(tSize, pstChunk->m_tPreSize))
	{
		...
		pstChunk = (MallocChunk *)(((char *)pstChunk) - pstChunk->m_tPreSize);
		// unlink
		unlink_bin(m_ptBin[uiBinIdx], ptr_to_handle(pstChunk));
		if (empty_bin(uiBinIdx))
			clear_bin_bit_map(uiBinIdx);
	...
	if (in_smallbin_range(tSize))
	{
		link_bin(m_ptBin[smallbin_index(tSize)], ptr_to_handle(pstChunk));
...
#endif
		link_bin(m_ptBin[largebin_index(tSize)], ptr_to_handle(pstChunk));
...
int DTCBinMalloc::Free(ALLOC_HANDLE_T hHandle)
{
	int iRet;
	ALLOC_SIZE_T tSize;
...
unsigned DTCBinMalloc::ask_for_destroy_size(ALLOC_HANDLE_T hHandle)
{
	...
	if (!prev_inuse(current_chunk) && CAN_COMBILE(physic_size, current_chunk->m_tPreSize))
	{
		...
		current_chunk = (MallocChunk *)((char *)current_chunk - current_chunk->m_tPreSize);
	}

	/* try combile next-chunk */
	if (physic_handle + physic_size != m_pstHead->m_hTop)
	{
		next_chunk = (MallocChunk *)(handle_to_ptr(physic_handle + physic_size));
...
ALLOC_SIZE_T DTCBinMalloc::last_free_size()
{
	free_fast();
