...
StatClient stc;
std::vector<StatClient::iterator> idset;
TableFormater out;
int outfmt = TableFormater::FORMAT_ALIGNED;
...
static inline void cell_name(const char *name)
{
	if (outnoname)
		return;
	if (outfmt == TableFormater::FORMAT_ALIGNED)
		out.Cell("%s:", name);
	else
		out.Cell("%s", name);
}
...
void dump_data(void)
{
	...
	for (i = 0; i < idset.size(); i++)
	{
		...
		case SA_SAMPLE:
			...
			cell_value(s->unit(), stc.read_sample_average(s, SCC_10S));
			cell_value(s->unit(), stc.read_sample_average(s, SCC_10M));
			cell_value(s->unit(), stc.read_sample_average(s, SCC_ALL));
			...
			if (nobase < 2)
			{
				...
				cell_value(SU_INT, stc.read_sample_counter(s, SCC_10S));
				cell_value(SU_INT, stc.read_sample_counter(s, SCC_10M));
				cell_value(SU_INT, stc.read_sample_counter(s, SCC_ALL));
			}
			if (nobase == 0)
			{
				...
				for (int n = 1; n <= sn; n++)
				{
					...
					cell_value(SU_INT, stc.read_sample_counter(s, SCC_10S, n));
					cell_value(SU_INT, stc.read_sample_counter(s, SCC_10M, n));
					cell_value(SU_INT, stc.read_sample_counter(s, SCC_ALL, n));
				}
			}
		...
		case SA_COUNT:
			...
			cell_value(s->unit(), stc.read_counter_value(s, SCC_10S));
			cell_value(s->unit(), stc.read_counter_value(s, SCC_10M));
			cell_value(s->unit(), stc.read_counter_value(s, SCC_ALL));
			break;
		case SA_VALUE:
			cell_id(s->id());
			cell_name(s->name());
...
{
	char buf[256];
	if (statmgr.CreateStatIndex("dtcd", STATIDX, StatDefinition, buf, sizeof(buf)) < 0)
	{
...
void parse_sample_id(int argc, char **argv)
{
	...
	for (; argc > 0; argc--, argv++)
	{
		...
		case 2:
			for (n = stc.begin(); n != stc.end(); n++)
			{
				if ((int)n->id() >= s && (int)n->id() <= e && n->is_sample())
					idset.push_back(n);
			}
			break;
		case 1:
	...
	while (argc > 0 && argv[0][0] == '-')
	{
		...
		while ((c = *++p))
		{
			...
			case 'r':
				...
				break;
			case 't':
				outfmt = TableFormater::FORMAT_TABBED;
				break;
			case 'c':
				outfmt = TableFormater::FORMAT_COMMA;
				break;
			case 'n':
				outnoname = 1;
				break;
