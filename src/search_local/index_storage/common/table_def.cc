...
const SectionDefinition tableAttributeDefinition = {
	DRequest::Section::table_definition, 4, {
												DField::None,
...
DTCTableDefinition::DTCTableDefinition(int m) : maxFields(m),
												usedFields(0),
												numFields(0),
												keyFields(0)
{
	...
	if (maxFields > 0)
	{
		...
		fieldList = (FieldDefinition *)calloc(maxFields, sizeof(FieldDefinition));
...
int DTCTableDefinition::add_field(int id, const char *name, uint8_t type, int size)
{
...
int DTCTableDefinition::Unpack(const char *ptr, int len)
{
	...
	maxFields = nf;
	fieldList = maxFields == 0 ? NULL : (FieldDefinition *)calloc(maxFields, sizeof(FieldDefinition));
