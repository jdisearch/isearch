...
int encoded_bytes_field_value(const DTCFieldValue &fv)
{
	...
	for (int i = 0; i < fv.num_fields(); i++)
	{
		...
		if (fv.field_id(i) == 0)
			len++;
		len += 2 + encoded_bytes_data_value(fv.field_value(i), fv.field_type(i));
	}
	return len;
}
...
char *encode_multi_key(char *p, const DTCValue *v, const DTCTableDefinition *tdef)
{
	if (tdef->key_fields() <= 1)
		return p;
	*p++ = tdef->key_fields() - 1;
...
int encoded_bytes_field_value(const FieldValueByName &fv)
{
	...
	for (int i = 0; i < fv.num_fields(); i++)
	{
		...
		len += encoded_bytes_data_value(fv.field_value(i), fv.field_type(i));
	}
	return len;
}
