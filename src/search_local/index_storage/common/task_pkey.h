...
{
public:
	static int packed_key_size(const char *p, int keyformat) { return keyformat ?: 1 + *(uint8_t *)p; }
	static void unpack_key(const DTCTableDefinition *, const char *, DTCValue *);
...
#error unkown endian
	...
	static int build_packed_key(const DTCTableDefinition *, const DTCValue *, unsigned int bufsz, char packKey[]);
	static unsigned long calculate_barrier_key(const DTCTableDefinition *tdef, const char *pkey);
};
