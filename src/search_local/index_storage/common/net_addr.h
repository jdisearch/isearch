...
public: // methods
	SocketAddress(void) : socktype(0), alen(0) { this->addr->sa_family = 0; }
	~SocketAddress(void) {}

	// initialize to name
	// success: return NULL
	// failure: return error message
	const char *set_address(const char *name, int port = 0, int type = SOCK_STREAM);
	...
	const char *set_address(const struct sockaddr_un *, int socklen, int type);
	const char *set_address(const struct sockaddr_in *, int socklen, int type);
	const char *set_address(const struct sockaddr_in6 *, int socklen, int type);
	int socket_family(void) const { return this->addr->sa_family; }
	int socket_type(void) const { return this->socktype; }
	const char *socket_type_name(void) const
	{
		return this->socktype == SOCK_DGRAM ? "DGRAM" : this->socktype == SOCK_STREAM ? "STREAM" : this->socktype == SOCK_RAW ? "RAW" : this->socktype == SOCK_SEQPACKET ? "SEQPACKET" : "<BAD>";
	}
	int create_socket(void) const { return socket(this->addr->sa_family, this->socktype, 0); }
	int connect_socket(int fd) const { return connect(fd, this->addr, this->alen); }
	int bind_socket(int fd) const { return bind(fd, this->addr, this->alen); }
...
private:
	...
	const char *set_ipv4_address(const char *name, int port = 0, int type = SOCK_STREAM);
	const char *set_ipv6_address(const char *name, int port = 0, int type = SOCK_STREAM);
	const char *set_host_address(const char *name, int port = 0, int type = SOCK_STREAM);
...
extern int sock_bind(const SocketAddress *addr, int backlog = 0, int rbufsz = 0, int wbufsz = 0, int reuse = 0, int nodelay = 0, int defer = 0);
#endif
