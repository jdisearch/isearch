...
int DTCFieldValue::Update(RowValue &r)
{
	for (int i = 0; i < num_fields(); i++)
	{
		...
		switch (TP(r.field_type(id), fieldValue[i].type, fieldValue[i].oper))
		{
		default:
			log_notice("id:%d TC(%d,%d,%d) not surport", id, r.field_type(id), fieldValue[i].type, fieldValue[i].oper);
			break;
		case TC(Signed, Signed, Set):
		case TC(Unsigned, Signed, Set):
		case TC(Unsigned, Unsigned, Set):
			v1.s64 = v2.s64;
			TruncateIntValue(v1, r.field_type(id), r.field_size(id));
			break;
...
int DTCFieldValue::Compare(const RowValue &r, int iCmpFirstNRows)
{
	...
	{
		const int id = fieldValue[i].id;
		if (id < r.table_definition()->key_fields() || id > iCmpFirstNRows - 1)
			continue;
...
int RowValue::Compare(const RowValue &rv, uint8_t *fieldIDList, uint8_t num)
{
	for (int i = 0; i < num; ++i)
	{
		...
		case DField::Unsigned:
			if (value[fieldIDList[i]].u64 != rv.field_value(fieldIDList[i])->u64)
				return -1;
			break;
		...
		case DField::String: //字符串
			if (!string_equal(value[fieldIDList[i]], *(rv.field_value(fieldIDList[i]))))
				return -1;
			break;
		case DField::Binary: //二进制数据
			if (!binary_equal(value[fieldIDList[i]], *(rv.field_value(fieldIDList[i]))))
				return -1;
			break;
		default:
			return -3;
