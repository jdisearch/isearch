...
struct SectionDefinition
{
	...
	uint8_t tag_type(uint8_t i) const { return tagType[i]; };
	int max_tags(void) const { return maxTags; }
	int section_id(void) const { return sectionId; }
};
...
public:
	...
	{
#if !MAX_STATIC_SECTION
		tagValue = (DTCValue *)malloc(orig.definition->max_tags(), sizeof(DTCValue));
		if (tagValue == NULL)
			throw std::bad_alloc();
#endif
		Copy(orig);
	...
	{
		if (tag_type(id) == DField::String || tag_type(id) == DField::Binary)
	...
	void set_key(const DTCValue &k) { set_tag(0, k); }
};
...
public:
	...
	const DTCBinary &table_name(void) const { return tagValue[1].str; }
	void set_table_name(const char *n) { set_tag(1, n); }
	const DTCBinary &table_hash(void) const { return tagValue[4].str; }
	void set_table_hash(const char *n) { set_tag(4, n, 16); }
	const DTCBinary &data_table_hash(void) const { return tagValue[2].str; }
	void set_data_table_hash(const char *n) { set_tag(2, n, 16); }

	const DTCBinary &CTLibVer(void) const { return tagValue[6].str; }
	const int CTLibIntVer(void) const
	{
		...
		{
			log_debug("multi task have no version info");
			return major;
		}
		...
		{
			major = buf - '0';
			log_debug("client major version:%d,version string:%s", major, CTLibVer().ptr);
			return major;
		}
		else
		{
	...
	int ReConnect(void) const { return tagValue[19].u64; }
	int key_type(void) const { return tagValue[9].u64; }
	void set_key_type(int n) { set_tag(9, n); }
...
#error MAX_STATIC_SECTION must >= 10
	...
	void set_serial_nr(uint64_t u) { set_tag(3, u); }
	const int keep_alive_timeout(void) const
	{
		return (int)tagValue[8].u64;
	}
	void set_keep_alive_timeout(uint32_t u) { set_tag(8, u); }

	void set_hot_backup_id(uint64_t u) { set_tag(14, u); }
	uint64_t hot_backup_id() const { return tagValue[14].u64; }
	void set_master_hb_timestamp(int64_t t) { set_tag(15, t); }
	int64_t master_hb_timestamp(void) const { return tagValue[15].s64; }
	void set_slave_hb_timestamp(int64_t t) { set_tag(16, t); }
	int64_t slave_hb_timestamp(void) const { return tagValue[16].s64; }
	uint64_t get_agent_client_id() const { return tagValue[17].u64; }
	void set_agent_client_id(uint64_t id) { set_tag(17, id); }

	void set_access_key(const char *token) { set_tag(18, token); }
	const DTCBinary &access_key(void) const { return tagValue[18].str; }
};
...
public:
	...
	void set_timeout(uint32_t n) { set_tag(1, (uint64_t)n << 10); }
	uint32_t limit_start(void) const { return tagValue[2].u64; }
	uint32_t limit_count(void) const { return tagValue[3].u64; }
	void set_limit_start(uint32_t n) { set_tag(2, (uint64_t)n); }
	void set_limit_count(uint32_t n) { set_tag(3, (uint64_t)n); }

	uint32_t admin_code() const { return tagValue[7].u64; }
	void set_admin_code(uint32_t code) { set_tag(7, (uint64_t)code); }
};
...
public:
	DTCResultInfo() : SimpleSection(resultInfoDefinition), szErrMsg(NULL), s_info(NULL), t_info(NULL) {}
	~DTCResultInfo()
	{
		FREE_CLEAR(szErrMsg);
		FREE_CLEAR(s_info);
	...
	int result_code(void) const { return tagValue[1].s64; }
	const char *error_from(void) const { return tagValue[2].str.ptr; }
	const char *error_message(void) const { return tagValue[3].str.ptr; }
	uint64_t affected_rows(void) const { return tagValue[4].s64; }
	void set_affected_rows(uint64_t nr) { set_tag(4, nr); }
	uint64_t total_rows(void) const { return tagValue[5].s64; }
	void set_total_rows(uint64_t nr) { set_tag(5, nr); }
	uint64_t insert_id(void) const { return tagValue[6].u64; }
	void set_insert_id(uint64_t nr) { set_tag(6, nr); }
	...
	void set_time_info(uint32_t nr) { set_tag(8, nr); }
	/*set hit flag by tomchen 20140604*/
	void set_hit_flag(uint32_t nr) { set_tag(9, nr); }
	uint32_t hit_flag() { return tagValue[9].s64; };
