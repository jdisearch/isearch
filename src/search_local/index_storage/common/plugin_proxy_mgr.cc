 ...
 *
 *    Description:  dtc plugin management.
 *
 *        Version:  1.0
...
PluginAgentManager::PluginAgentManager(void) : _dll(NULL),
                                               _config(NULL),
                                               _plugin_name(NULL),
...
void PluginAgentManager::Destroy(void)
{
    return Singleton<PluginAgentManager>::Destroy();
}
...
int PluginAgentManager::open()
{
    ...
    if (NULL != _dll->handle_init)
    {
        ...
        log_debug("plugin::handle_init _plugin_conf_file=%s", (const char *)PluginGlobal::_plugin_conf_file);
    }
    ...
    {
        log_error("create PluginAgentListenerPool object failed, msg:%s", strerror(errno));
        return -1;
    }
