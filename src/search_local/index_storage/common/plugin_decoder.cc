...
int PluginReceiver::parse_protocol(PluginStream *request)
{
    ...
    log_debug("enter PluginReceiver::parse_protocol!!!!!!!!");
    ...
    if (real_len < 0)
    {
        ...
        log_error("invoke handle_input failed, fd[%d], length[%d], net stage[%d]", _fd, recv_len, _stage);
        return _stage;
    }
...
parse_out:
    if (_all_len_changed)
    {
        ...
        {
            _stage = NET_FATAL_ERROR;
            log_error("recv error, fd[%d], length[%d], net stage[%d], msg:%s", _fd, recv_len, _stage, strerror(errno));
        }
    }
...
int PluginReceiver::recv(PluginStream *request)
{
    ...
    if ((NET_IDLE == _stage) && (NULL == recv_buf))
    {
        ...
        {
            _stage = NET_FATAL_ERROR;
            log_error("malloc memory failed, size[%d], msg:%s", max_recv_len, strerror(errno));
            return _stage;
        }
    ...
    case -1:
        ...
        {
            _stage = NET_FATAL_ERROR;
            log_error("recv failed from fd[%d], net stage[%d], msg[%s]", _fd, _stage, strerror(errno));
            return _stage;
        }
        return _stage;
...
int PluginReceiver::recvfrom(PluginDatagram *request, int mtu)
{
    ...
    if (NULL == recv_buf)
    {
        log_error("allocate packet buffer[%d] failed, msg[no enough memory]", recv_len);
        return -1;
    }

    void *&req_addr = request->_addr;
    socklen_t &req_addr_len = request->_addr_len;
    recv_len = ::recvfrom(_fd, recv_buf, max_len, MSG_DONTWAIT | MSG_TRUNC, (struct sockaddr *)req_addr, &req_addr_len);

    if (recv_len <= 0)
    {
        if (errno != EAGAIN)
        {
            log_notice("recvfrom error: size=%d errno=%d", recv_len, errno);
        }

        return -1;
    }

    request->_skinfo.remote_ip = ((struct sockaddr_in *)req_addr)->sin_addr.s_addr;
    request->_skinfo.remote_port = ((struct sockaddr_in *)req_addr)->sin_port;

    real_len = _dll->handle_input(recv_buf, recv_len, &(request->_skinfo));

    if ((real_len <= 0) || (real_len > recv_len))
    {
        log_notice("the recv buffer is invalid.");
        return -1;
    }
...
int PluginSender::send(PluginStream *request)
{
    ...
    {
        _stage = NET_FATAL_ERROR;
        log_error("send length[%d] and send buffer[%p] is invalid.", send_len, send_buf);
        return _stage;
    }
    ...
    if (ret < 0)
    {
        ...
        {
            _stage = NET_SENDING;
            log_debug("*STEP: send data pending, _fd[%d], net stage[%d], sendlen[%d], sentlen[%d]",
        ...
        _stage = NET_FATAL_ERROR;
        log_error("*STEP: send data error, _fd[%d], net stage[%d], msg[%m]", _fd, _stage);
        return _stage;
    }
    ...
    if (sent_len + ret == send_len)
    {
        ...
        log_debug("*STEP: send data done, fd[%d], net stage[%d], sent len[%d], send len[%d]", _fd, _stage, sent_len, send_len);
        return _stage;
    }
    ...
    if (sent_len + ret < send_len)
    {
        sent_len += ret;
        _stage = NET_SENDING;
        log_debug("*STEP: sending data, fd[%d], net stage[%d], sent len[%d], send len[%d]", _fd, _stage, sent_len, send_len);
        return _stage;
    }

    _stage = NET_FATAL_ERROR;
    log_error("*STEP: send data exception, fd[%d], net stage[%d], sent len[%d], send len[%d]", _fd, _stage, sent_len + ret, send_len);
...
int PluginSender::sendto(PluginDatagram *request)
{
    ...
    {
        log_error("sendto data to client failed, sent length:%d, send length:%d, msg:%s", sent_len, send_len, strerror(errno));
        return -1;
    }
