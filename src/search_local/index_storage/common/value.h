...
struct DTCBinary
{
	...
	// API
	inline int is_empty(void) const { return len <= 0; };
	inline uint8_t unsigned_value(int n = 0) const { return *(uint8_t *)(ptr + n); }
	inline int8_t signed_value(int n = 0) const { return *(uint8_t *)(ptr + n); }
	...
	inline DTCBinary &operator=(const char *v) { return Set(v); }
...
typedef union DTCValue
{
	...
	DTCValue(const char *v) { str = DTCBinary::Make(v); }
	DTCValue(const char *v, int l) { str = DTCBinary::Make(v, l); }
	~DTCValue() {}
#if !__x86_64__
