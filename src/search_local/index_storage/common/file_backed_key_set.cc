 ...
 * =====================================================================================
 *
 *       Filename:  file_backed_key_set.cc
 *
 *    Description:  
 *
 *        Version:  1.0
...
FileBackedKeySet::hash_node *FileBackedKeySet::hash_node_allocator::alloc()
{
    if (!m_freeNode)
    {
        FileBackedKeySet::hash_node *nodes = (hash_node *)calloc(GROW_COUNT, sizeof(hash_node));
...
FileBackedKeySet::FileBackedKeySet(const char *file, int keySize) : m_filePath(file), m_fd(-1), m_keySize(keySize), m_base((char *)MAP_FAILED),
                                                                    m_buckets(0)
{
}
...
int FileBackedKeySet::Insert(const char *key)
{
...
bool FileBackedKeySet::Contains(const char *key, bool checkStatus)
{
    ...
    while (n)
    {
        ...
        {
            if ((checkStatus == true && *(k - 1) == MIGRATE_SUCCESS) ||
                checkStatus == false)
                return true;
            break;
        }
