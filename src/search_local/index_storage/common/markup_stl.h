// Markup.h: interface for the MarkupSTL class.
//
// Markup Release 6.3
// Copyright (C) 1999-2002 First Objective Software, Inc. All rights reserved
// Go to www.firstobject.com for the latest MarkupSTL and EDOM documentation
...
{
public:
	MarkupSTL() { set_doc(NULL); };
	MarkupSTL(const char *szDoc) { set_doc(szDoc); };
	MarkupSTL(const MarkupSTL &markup) { *this = markup; };
	...
	bool into_elem();
	bool out_of_elem();
	void reset_child_pos() { x_SetPos(m_iPosParent, m_iPos, 0); };

	void reset_main_pos() { x_SetPos(m_iPosParent, 0, 0); };
	void reset_pos() { x_SetPos(0, 0, 0); };
	std::string get_tag_name() const;
	std::string get_child_tag_name() const { return x_GetTagName(m_iPosChild); };
	std::string get_data() const { return x_GetData(m_iPos); };
	std::string get_child_data() const { return x_GetData(m_iPosChild); };
	//[CMARKUPDEV
	std::string find_get_data(const char *szName);
	//]CMARKUPDEV
	std::string get_attrib(const char *szAttrib) const { return x_GetAttrib(m_iPos, szAttrib); };
	std::string get_child_attrib(const char *szAttrib) const { return x_GetAttrib(m_iPosChild, szAttrib); };
	...
	std::string get_error() const { return m_strError; };
	...
	int get_node_type() { return m_nNodeType; };
	bool add_node(int nType, const char *szText) { return x_AddNode(nType, szText, false); };
	bool insert_node(int nType, const char *szText) { return x_AddNode(nType, szText, true); };
	bool remove_node();
	//]CMARKUPDEV

	// Create
	bool Save(const char *szFileName);
	std::string get_doc() const { return m_strDoc; };
	bool add_elem(const char *szName, const char *szData = NULL) { return x_AddElem(szName, szData, false, false); };
	bool insert_elem(const char *szName, const char *szData = NULL) { return x_AddElem(szName, szData, true, false); };
	bool add_child_elem(const char *szName, const char *szData = NULL) { return x_AddElem(szName, szData, false, true); };
	bool insert_child_elem(const char *szName, const char *szData = NULL) { return x_AddElem(szName, szData, true, true); };
	bool add_attrib(const char *szAttrib, const char *szValue) { return x_SetAttrib(m_iPos, szAttrib, szValue); };
	bool add_child_attrib(const char *szAttrib, const char *szValue) { return x_SetAttrib(m_iPosChild, szAttrib, szValue); };
	bool add_attrib(const char *szAttrib, int nValue) { return x_SetAttrib(m_iPos, szAttrib, nValue); };
	bool add_child_attrib(const char *szAttrib, int nValue) { return x_SetAttrib(m_iPosChild, szAttrib, nValue); };
	bool add_child_sub_doc(const char *szSubDoc) { return x_AddSubDoc(szSubDoc, false, true); };
	bool insert_child_sub_doc(const char *szSubDoc) { return x_AddSubDoc(szSubDoc, true, true); };
	std::string get_child_sub_doc() const;

	// Modify
	bool remove_elem();
	bool remove_child_elem();
	bool set_attrib(const char *szAttrib, const char *szValue) { return x_SetAttrib(m_iPos, szAttrib, szValue); };
	bool set_child_attrib(const char *szAttrib, const char *szValue) { return x_SetAttrib(m_iPosChild, szAttrib, szValue); };
	bool set_attrib(const char *szAttrib, int nValue) { return x_SetAttrib(m_iPos, szAttrib, nValue); };
	bool set_child_attrib(const char *szAttrib, int nValue) { return x_SetAttrib(m_iPosChild, szAttrib, nValue); };
	//[CMARKUPDEV
	bool remove_attrib(const char *szAttrib) { return x_RemoveAttrib(m_iPos, szAttrib); };
	bool remove_child_attrib(const char *szAttrib) { return x_RemoveAttrib(m_iPosChild, szAttrib); };
	bool find_set_data(const char *szName, const char *szData, int nCDATA = 0);
	//]CMARKUPDEV
	bool set_data(const char *szData, int nCDATA = 0) { return x_SetData(m_iPos, szData, nCDATA); };
	bool set_child_data(const char *szData, int nCDATA = 0) { return x_SetData(m_iPosChild, szData, nCDATA); };
...
protected:
	...
	struct ElemPos
	{
		...
		bool is_empty_element() const { return (nStartR == nEndL + 1); };
		void Clear()
		{
			nStartL = 0;
			nStartR = 0;
