...
int CAgentListenPkg::Bind(const char *bindaddr, CTaskDispatcher<CTaskRequest> * agentprocess, int32_t listen_fd)
{
//    char bindstr[64];
...
//    blog = gc->GetIntVal("cache", "AgentListenBlog", 256);
	...
	thread = new CPollThread(threadname);
	if(NULL == thread)
	{
	...
	if (listen_fd <=0) {
		...
		listener->SetListenNetfd(listen_fd);
	}
...
int CAgentListenPkg::GetListenFd()
{
	if (listener == NULL)
		return -1;
...
void CAgentListenPkg::CloseListenFd()
{
	if (listener == NULL)
		return ;
	listener->CloseListenFd();
	listener = NULL;
}