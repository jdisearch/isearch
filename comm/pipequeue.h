 ...
 *
 *    Description:  pipequeue class definition.
 *
 *        Version:  1.0
 *        Created:  09/08/2018
 *       Revision:  none
...
public:
	...
	inline int AttachPoller (CPollThread *thread, int fd)
	{
		owner = thread;
		netfd = fd;
		return CPollerObject::AttachPoller(thread);
...
public:
	inline CPipeQueue(){ }
	inline ~CPipeQueue(){}
	inline int AttachPoller(CPollThread *fr, CPollThread *to, int fd[2])
	{
		CPipeQueueReader<T, C>::AttachPoller(to, fd[0]);
		writer.AttachPoller(fr, fd[1]);
		return 0;
	}
	inline int AttachPoller(CPollThread *fr, CPollThread *to)
	{
		int fd[2];
		int ret = pipe(fd);
        if(ret != 0)
